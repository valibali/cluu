...
pub extern "C" fn main() -> i32 {
    ...
    let _ = libcluu::debug_print(&format!("{}: first SET_LEADER({}) = {:?}\n", label, my_pid, result));
    if result.is_err() {
        let _ = libcluu::debug_print(&format!("{}: first SET_LEADER failed {:?}\n", label, result.err()));
        let _ = libcluu::session::destroy(ok.token);
        return 1;
    }

    let second = libcluu::session::set_leader(ok.token, my_pid + 1);
    let _ = libcluu::debug_print(&format!("{}: second SET_LEADER({}) = {:?}\n", label, my_pid + 1, second));
    match second {
        Ok(()) => {
            let _ = libcluu::debug_print(&format!("{}: second SET_LEADER should have failed\n", label));
            let _ = libcluu::session::destroy(ok.token);
            return 1;
        }
        Err(SessionErr::AlreadyHasLeader) => {
            let _ = libcluu::debug_print(&format!("{}: second SET_LEADER correctly denied\n", label));
        }
        Err(e) => {
            let _ = libcluu::debug_print(&format!("{}: second SET_LEADER wrong error {:?}\n", label, e));
            let _ = libcluu::session::destroy(ok.token);
            return 1;
        }
    }
