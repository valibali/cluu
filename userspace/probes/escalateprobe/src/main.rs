...
fn run() -> libcluu::Result<()> {
    debug_print("escalateprobe: start")?;

    registry::init("escalateprobe")?;
    syscall::yield_cpu()?;

    let procmgr_ep = registry::subscribe_output("root-procmgr", "spawn")?;
    debug_print("escalateprobe: got procmgr endpoint")?;

    // Try to spawn "devicereq" which requires DEVICE capability.
    // escalateprobe's profile is ipc+vfs+registry (no DEVICE),
    // so procmgr should reject with PermissionDenied.
    let payload = b"devicereq";
    ...
    if status != 0 {
        let _ = debug_print(&alloc::format!(
            "escalateprobe: PASS escalation denied status={}",
            status
        ));
    } else {
        let _ = debug_print("escalateprobe: FAIL escalation was not denied");
