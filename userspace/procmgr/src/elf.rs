...
//!
//! Bottom-up implementation: Start with just what we need to load a program.
//! This is NOT a full ELF parser - just enough to load x86_64 executables.
...
impl ElfFile {
    ...
    pub fn parse(data: &[u8]) -> Result<Self> {
        ...
        if data.len() < core::mem::size_of::<Elf64Ehdr>() {
            libcluu::debug_print("ELF file too small for header")?;
        ...
        if phoff + (phnum * phentsize) > data.len() {
            libcluu::debug_print("Program headers out of bounds")?;
            return Err(libcluu::Error::InvalidArgument);
        }
        ...
        for i in 0..phnum {
            if elf.segment_count >= 8 {
                libcluu::debug_print("Too many segments (max 8)")?;
                break;
            }
        ...
        if elf.segment_count == 0 {
            libcluu::debug_print("No loadable segments found")?;
            return Err(libcluu::Error::InvalidArgument);
        }
    ...
    fn validate_header(ehdr: &Elf64Ehdr) -> Result<()> {
        ...
        if ehdr.e_ident[0..4] != ELF_MAGIC {
            libcluu::debug_print("Invalid ELF magic")?;
            return Err(libcluu::Error::InvalidArgument);
        }

        // Check class (64-bit)
        if ehdr.e_ident[4] != ELFCLASS64 {
            libcluu::debug_print("Not a 64-bit ELF")?;
            return Err(libcluu::Error::InvalidArgument);
        }

        // Check endianness (little-endian)
        if ehdr.e_ident[5] != ELFDATA2LSB {
            libcluu::debug_print("Not little-endian ELF")?;
            return Err(libcluu::Error::InvalidArgument);
        }

        // Check type (executable)
        if ehdr.e_type != ET_EXEC {
            libcluu::debug_print("Not an executable ELF")?;
            return Err(libcluu::Error::InvalidArgument);
        }

        // Check machine (x86-64)
        if ehdr.e_machine != EM_X86_64 {
            libcluu::debug_print("Not an x86-64 ELF")?;
            return Err(libcluu::Error::InvalidArgument);
        }

        // Check entry point is valid
        if ehdr.e_entry == 0 {
            libcluu::debug_print("Invalid entry point")?;
            return Err(libcluu::Error::InvalidArgument);
        }
