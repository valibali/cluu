...
impl RestartTracker {
    ...
    pub fn register(&mut self, key: u64, policy: Policy) {
        self.table.insert(key, Entry { attempts: 0, first_attempt: 0, policy });
    }
    pub fn on_exit(&mut self, key: u64, exit_code: i32, now: u64) -> Decision {
        let e = match self.table.get_mut(&key) {
            Some(e) => e, None => return Decision::NoRestart,
        };
        let want = match e.policy {
            Policy::Never => false,
            Policy::Always => true,
            Policy::OnFailure => exit_code != 0,
        };
        if !want { return Decision::NoRestart; }
        if e.attempts == 0 { e.first_attempt = now; }
        e.attempts += 1;
        if now.saturating_sub(e.first_attempt) > WINDOW_TICKS {
            e.attempts = 1; e.first_attempt = now;
        }
...
mod tests {
    ...
    #[test]
    fn always_restarts_to_threshold() {
        let mut t = RestartTracker::new();
