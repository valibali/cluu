...
/* CLUU userspace platform. Set via -D__CLUU__=1 on the compile command line.
   Not auto-detected from host preprocessor symbols: CLUU cross-compiles from
   Linux but targets its own userspace, so host auto-detection would mis-identify
   it as Linux and pull in Linux backends. */
#if defined(__CLUU__)
#undef __CLUU__
#define __CLUU__ 1
#endif
