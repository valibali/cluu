...
## D. Plan 1 spawn() bypass retire

**Goal:** delete `handle_spawn_unified`'s short-circuit (`70a4418`); route every spawn through the real `procmgr::spawn()` hooks defined in `docs/superpowers/plans/2026-05-18-plan1-unified-spawn-protocol.md` Tasks 8–10.
...
**Steps:**
1. Audit: list every call site that hits the bypass path. `grep -rn 'handle_spawn_unified' userspace/root-procmgr userspace/session-procmgr` + comment marks.
2. Land Plan 1 Tasks 8 + 9 (real `spawn()` and `PROCMGR_SPAWN_UNIFIED_LABEL` dispatch) inside the new procmgr crate split.
3. Add `cargo xtask check-cap-purity` lint rule: forbid new references to the bypass once Plan 1 Tasks 8/9 land. Removes the temptation to add new callers.
4. Cutover one caller at a time (login → cluuterm → shell → primordial seed paths). Build green per cutover.
5. Delete the bypass + comment.
6. Coverage check: `cargo xtask coverage-check` ≥ 95% line + branch for both procmgr crates.
...
**Acceptance:** zero call sites reach the bypass; cap-purity gate green; coverage gate green. **Estimated cost:** 4–5 days.
...
A (1d) + B (2d) + C (8–10d) + D (4–5d) = **15–18 working days**. Phase 5 (Network) starts after.
