...
## Phase 0 — Pre-flight verifications (DONE 2026-04-29)

### T0 Findings (recorded)

These shape downstream tasks — read before implementing T10, T18, T34.

1. **No native `libcluu` raw-mode helper.** `posix::tcsetattr` only marshals `c_lflag`. The shell's private helpers `tty_get_lflag` / `tty_set_lflag` (`userspace/shell/src/commands.rs:1629-1641`) are the actual pattern — single `TTY_CTL_LABEL` call, subcmd=0 read / subcmd=1 write, `words[4]` carries the lflag. The editor should either copy them or promote to `libcluu::posix` (~30 LOC). **Recommend promoting** so future TUI binaries don't reinvent.
2. **TTY constants:** `TTY_LFLAG_ICANON = 0x02`, `TTY_LFLAG_ECHO = 0x08`. "Raw mode" = clear both. Source: `userspace/shell/src/commands.rs:35-37`.
3. **Input model:** the editor receives `TTY_READ_LABEL` IPC messages with payload bytes — same as the shell's main loop at `userspace/shell/src/main.rs:142-162`. `recv` on `info.tokens[TOKEN_STDIN]` and parse the payload. **In raw mode, the line-discipline emits `0x1B`, `[`, `A` as separate raw bytes** — the editor's CSI parser owns the decoding (the TTY's CSI parser only runs in canonical mode for echo).
4. **Console SGR / cursor escapes** (`userspace/console/src/renderer.rs:269-366`):
   - **Supported:** `CSI r;c H`, `CSI A/B/C/D` (cursor moves with count), `CSI K`, `CSI 2 J`, `CSI 0 m`, `CSI 30..=37 m` / `40..=47 m` / `90..=97 m` / `100..=107 m`.
   - **NOT supported (silently consumed):**
     - `CSI 7 m` (reverse video) → use a colored bg instead, e.g. `CSI 47;30m` (white bg + black fg) for status line.
     - `CSI ?25 l` / `CSI ?25 h` (cursor hide/show) → don't use; just position cursor at end of frame.
     - `CSI 39 m` / `CSI 49 m` (default fg/bg) → use `CSI 0 m` to reset all attrs instead.
     - `CSI 1 m` (bold), `CSI 4 m` (underline) → no-op.
5. **VFS rename:** `VfsClient::rename(old, new) -> Result<()>` at `userspace/libcluu/src/fs/client.rs:317-332`. Suitable for atomic save.
6. **Whole-file read pattern:** copy `read_file_via_vfs` from `userspace/shell/src/shellrc.rs:103-162`. Uses `VSPACE.lock().alloc(chunk_alloc)` + `vfs.read_grant(...)` in a 4KB loop. Editor will use a 1MB cap instead of shellrc's 64KB cap.
7. **VFS write:** `VfsClient::write(file, offset, data)` at `client.rs:251-261`. No "write whole file" helper exists — single `vfs.write(file, 0, &bytes)` call works for files under 1MB. Open with `O_WRONLY | O_CREAT | O_TRUNC` and mode `0o644`. **Sequence for atomic save:** `open_with(tmp, ...)` → `write(file, 0, &bytes)` → `close(file)` → `rename(tmp, final)`.
8. **Harness keystroke injection limit:** `KEYSTROKE_COMMANDS` always types whole lines + Enter; `POST_SENDKEY` sends one key. There's **no native path for sending escape-sequence byte streams** (e.g. `iHelloEsc:wqEnter` as a single payload). **Workaround for `l2_edit_*` cases — drive the editor from a parent shell that injects bytes via `send_with_payload(child_stdin, TTY_READ_LABEL, ...)`**, the same pattern SuBuiltin uses at `userspace/shell/src/commands.rs:3027-3060`. Update T34 to use this.
9. **Task #80 (pending):** VFS open(O_WRONLY|O_CREAT) sometimes times out on shell's MemFs `/tmp`. **Editor harness cases that test save should target ext2-backed paths (`/home/root/...`)**, not `/tmp`, until task #80 is resolved.
...
### Task 10: Raw-mode TTY setup/teardown via TTY_CTL_LABEL

Per T0 finding 1: libcluu doesn't have a native raw-mode helper. The shell uses private `tty_get_lflag` / `tty_set_lflag` helpers — we'll promote them to `libcluu::posix::tty` so the editor (and any future TUI) can reuse.

**Files:**
- Modify: `userspace/libcluu/src/posix/mod.rs` (re-export new submodule)
- Create: `userspace/libcluu/src/posix/tty.rs` — public `get_lflag` / `set_lflag` / `enter_raw` / `restore` helpers
- Create: `userspace/edit/src/tty.rs` — thin wrappers over libcluu helpers
- Modify: `userspace/edit/src/main.rs` (declare `mod tty;`)

- [ ] **Step 1: Add `userspace/libcluu/src/posix/tty.rs`**

Mirror the shell's private helpers (`userspace/shell/src/commands.rs:1629-1641`):

```rust
//! TTY line-discipline helpers (raw mode for TUI binaries).
//!
//! Marshalled over `TTY_CTL_LABEL` — single message, words=5, words[0]=subcmd
...
use crate::ipc::{Message, IpcFlags, send, call, TTY_CTL_LABEL};
use crate::Result;
...
pub fn get_lflag(tty_endpoint: usize) -> Result<usize> {
    ...
    msg.words[0] = 0;            // subcmd: get
    let mut reply = Message::new(0, [0; 6], 0);
    call(tty_endpoint, &msg, &mut reply, IpcFlags::empty())?;
    Ok(reply.words[4])
}
...
pub fn set_lflag(tty_endpoint: usize, lflag: usize) -> Result<()> {
    let mut msg = Message::new(TTY_CTL_LABEL, [0; 6], 5);
    msg.words[0] = 1;            // subcmd: set
    msg.words[4] = lflag;
    send(tty_endpoint, &msg, IpcFlags::empty())
}
...
pub fn enter_raw(tty_endpoint: usize) -> Result<SavedTty> {
    ...
    Ok(SavedTty { tty_endpoint, saved_lflag })
}
...
The exact import paths (`Message`, `IpcFlags`, `send`, `call`, `TTY_CTL_LABEL`) need to match libcluu's actual module layout. Adapt if the names differ.

- [ ] **Step 2: Re-export from `posix/mod.rs`**

Add `pub mod tty;` to `userspace/libcluu/src/posix/mod.rs`.

- [ ] **Step 3: Update `userspace/shell/src/commands.rs` to use the libcluu helpers**

Find the private `tty_get_lflag` / `tty_set_lflag` (around line 1629). Replace internal use with `libcluu::posix::tty::get_lflag` / `set_lflag` (or leave the privates as thin wrappers that call libcluu — pick whichever needs less churn). Keep this scope tight: don't refactor the shell's wait-for-exit logic.

- [ ] **Step 4: Create `userspace/edit/src/tty.rs`**
...
Run: `cargo xtask build`
Expected: success — both libcluu and shell rebuild against the new helpers, edit's tty.rs compiles.

- [ ] **Step 6: Commit**

```bash
git add userspace/libcluu/src/posix/ userspace/shell/src/commands.rs userspace/edit/src/main.rs userspace/edit/src/tty.rs
git commit -m "edit/tty: promote tty_get_lflag/set_lflag to libcluu::posix::tty; editor wrapper — T10"
```
...
**Per T0 finding 8: the harness `KEYSTROKE_COMMANDS` mechanism types whole lines + Enter — it can't send escape sequences mid-line (e.g. `i…hello…Esc…:wq…Enter` as one payload). The right approach for editor cases is to inject bytes into the editor's stdin via a parent shell using SuBuiltin's payload-injection pattern (see `userspace/shell/src/commands.rs:3027-3060` — `send_with_payload(child_stdin, TTY_READ_LABEL, ...)`).**

Concretely: write a tiny `editprobe` helper binary (similar to `pipeprobe`) that:
1. Calls `procmgr::container_run` to spawn `edit /home/root/test.txt` (or whatever).
2. Captures the child's stdin endpoint from the reply.
3. Sends a fixed byte sequence: `iHelloESC:wq\n` (with literal Esc byte 0x1B between `Hello` and `:`).
4. Waits for the child to exit, prints `editprobe: PASS`/`FAIL`.

Or: a simpler approach for `l2_edit_eacces` — the test just needs to verify that `:w` against /etc fails. Use `su alice -c edit /etc/motd` and rely on the `KEYSTROKE_COMMANDS` for the followup `:w` line, exploiting the fact that the editor's parser reads a whole line of `:w` + Enter the same way the shell does for ex commands.

**This step may need a separate prep task** to build `editprobe` if straight keystroke injection doesn't work. If that's the case, flag DONE_WITH_CONCERNS and add a Task 36 to your TodoWrite.

Expected: all PASS once injection is wired correctly. If a case is fundamentally not injectable through the harness, we accept that case as "manual verification only" and document it.
