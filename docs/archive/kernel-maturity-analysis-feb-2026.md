...
| M6 (Weeks 12-14) | IPC throughput hardening | Remove fixed-4KB/two-copy IPC hot path for common traffic |
| M7 (Week 15) | Level 1 closeout | Stabilization, documentation, and release readiness review |
...
| WP-L2.1 Mutable FS operations + DAC checks | L2A | IN PROGRESS (`write` + append/grow ext2 writes landed; mutable ops wired end-to-end for ext2 backend: `mkdir/rename/rmdir/unlink`; `O_CREAT` path now creates files through VFS->blk->ext2; VFS now tracks per-path owner (authenticated sender) and enforces owner checks for mutate/create flows in addition to protected-path denies; explicit multi-client non-owner deny coverage landed via spawned `ownerprobe` flow) | `userspace/ext2`, `userspace/virtio-blk`, `userspace/vfs`, `userspace/libcluu`, `userspace/shell`, `userspace/c-programs`, `xtask`, `test_hello.sh` | `MARKER_MODE=l2_ext2write ./test_hello.sh --no-build`, `MARKER_MODE=l2_ext2append ./test_hello.sh --no-build`, `MARKER_MODE=l2_ext2mutate ./test_hello.sh --no-build`, `MARKER_MODE=l2_ext2unlink ./test_hello.sh --no-build`, `MARKER_MODE=l2_owner_deny ./test_hello.sh --no-build` pass |
| WP-L2.2 Minimal signals + shell job control | L2B | IN PROGRESS (canonical-mode `Ctrl-C` interrupt marker + foreground `spawn` `SIGINT` path landed; background baseline landed via `spawnbg`/`jobs` + async reap notifications (`shell: bg done pid=...`) from nonblocking notify polling; kernel-backed thread suspend/resume invoke path is now wired and procmgr routes `SIGSTOP`/`SIGCONT` non-destructively while keeping terminate signals destructive; shell job model now tracks `running/stopped` state with `stop`, `bg`, and `fg` continue-on-foreground behavior; added `jobchurn` builtin to run repeated stop/resume/foreground cycles and extended kernel telemetry with suspend/resume success counters to make this measurable in harness logs; state-transition paths now use explicit shell-side invariant checks when mutating background job state; added heavier churn harness mode (`l2_jobchurn_heavy`) to stress repeated transition loops; added deterministic multi-job interleaving builtin/harness path (`jobmix` / `l2_jobmix`) to exercise stop/continue/foreground ordering across two background jobs; libc `waitpid` now has explicit queued child-exit semantics with `WNOHANG` support for `SIGCHLD`-style polling and a dedicated userspace probe (`waitprobe`) in harness mode; remaining: expand from deterministic interleavings to async race-focused multi-job matrices) | `userspace/tty`, `userspace/shell`, `userspace/procmgr`, `userspace/libcluu`, `kernel/src/sched/*`, `kernel/src/syscall/handlers.rs`, `kernel/src/telemetry.rs`, `userspace/c-programs`, `xtask`, `test_hello.sh`, `scripts/harness_matrix.sh` | `MARKER_MODE=l2_sigint ./test_hello.sh`, `MARKER_MODE=l2_jobs ./test_hello.sh`, `MARKER_MODE=l2_fg ./test_hello.sh`, `MARKER_MODE=l2_stop ./test_hello.sh`, `MARKER_MODE=l2_jobchurn ./test_hello.sh`, `MARKER_MODE=l2_jobchurn_heavy ./test_hello.sh`, `MARKER_MODE=l2_jobmix ./test_hello.sh`, and `MARKER_MODE=l2_waitpid ./test_hello.sh` pass with full rebuild |
| WP-L2.3 Real `mmap` allocator + `mprotect` | L2C | IN PROGRESS (userspace mmap now uses tracked first-fit allocation with overlap checks and free-hole reuse; `munmap` validates exact tracked regions; `mprotect` routes through kernel-backed `InvokeOp::SpaceProtect` page retagging before userspace metadata updates; dedicated userspace probe + harness marker mode now validates RW->RX->RW transitions, PROT_NONE rejection path, and hole reuse behavior) | `kernel/src/token/mod.rs`, `kernel/src/syscall/handlers.rs`, `userspace/libcluu/src/syscall.rs`, `userspace/libcluu/src/posix/memory.rs`, `userspace/c-programs/mmapprobe.c`, `xtask/src/main.rs`, `test_hello.sh`, `scripts/harness_matrix.sh`, kernel VM path | `MARKER_MODE=l2_mmap ./test_hello.sh` passes with full rebuild (`mmapprobe: PASS ...` markers including `mprotect to rx` and restore-RW) |
| WP-A1.1 POSIX compatibility stubs (`signal`/`sigaction`/`raise`, `fcntl`) | A1 | IN PROGRESS (added minimal userspace signal registry + default-to-`_kill` raise path and `fcntl` support for `F_GETFL/F_SETFL/F_DUPFD`) | `userspace/libcluu/src/posix/signal.rs`, `userspace/libcluu/src/posix/fcntl.rs`, `userspace/libcluu/src/posix/mod.rs` | `cargo check -p libcluu` + new C probe (`a_signal_fcntl`) to be added to harness |
| WP-A2.1 `poll(2)` compatibility baseline | A2 | IN PROGRESS (added conservative userspace `poll` over FD table semantics + C probe and harness mode) | `userspace/libcluu/src/posix/poll.rs`, `userspace/libcluu/src/posix/mod.rs`, `userspace/c-programs/pollprobe.c`, `xtask/src/main.rs`, `test_hello.sh`, `scripts/harness_cases.conf` | `MARKER_MODE=a_poll ./test_hello.sh` (full build) |
| WP-B3.1 Spawn hot-path performance pass (procmgr/VFS orchestration) | B3 | IN PROGRESS (spawn-path control-plane overhead reduced: cached `procmgr:spawn` lookup in `libcluu` + corrected `PROC_KILL` label; harness now has dedicated `b_spawn_perf` mode. Added timestamped spawn stage traces in procmgr (`spawn_request`, `space_create_done`, `elf_fetch_start/done`, `map_segments_done`, `stack_map_done`, `thread_start_done`, `reply_sent`) and VFS (`map_elf_trace` request/cache/mapped/reply). Implemented two data-plane increments: procmgr shared-ring fallback loader path for VFS file reads and wider VFS<->blkdev grant windows (1MiB -> 4MiB) for cold ELF cache fills. Full-build measurement now shows reduced cold-cache map_elf cost (recent run examples: `/bin/hello` ~4.01B -> ~3.34B cycles, `/bin/benchprobe` ~4.29B -> ~3.28B, `/bin/sleepy` ~5.38B -> ~4.79B) with `benchprobe spawn_wait` at ~216.7M in the latest run; remaining issue is variance across runs, so next B3 step is cache warmup/metadata reuse stabilization) | `userspace/libcluu/src/posix/process.rs`, `userspace/libcluu/src/registry.rs`, `userspace/procmgr/src/main.rs`, `userspace/vfs/src/main.rs`, `userspace/virtio-blk/src/main.rs`, `userspace/c-programs/benchprobe.c`, `test_hello.sh`, `scripts/harness_cases.conf` | `MARKER_MODE=b_spawn_perf ./test_hello.sh` (full build) for spawn metric + stage-trace capture; keep `perf_benchprobe` for full mixed probe |
| WP-M6.1 IPC compact-message storage (inline fast path) | M6 | IN PROGRESS (kernel queue storage no longer uses fixed 4KB per message: `EndpointMessage` now stores either inline small payload (<=64B) or exact-size boxed payload; send/call user-copy paths now allocate by message length instead of unconditional 4KB stack buffers; queue telemetry now confirms compact-path queue pressure stays in low-KiB range under spawn churn) | `kernel/src/ipc/endpoint.rs`, `kernel/src/telemetry.rs`, `test_hello.sh`, `scripts/harness_cases.conf` | `MARKER_MODE=m6_ipc_compact CLUU_BOOTBOOT_ENV=cluu.ipc_direct=0 TEST_COMMAND_REPEAT=1 RUN_WAIT=16 MIN_EXIT_COOKIES=6 MAX_IPC_WAIT_P95_MS=16 MAX_IPC_WAIT_P99_MS=16 MAX_IPC_SCAN_AVG_STEPS_X100=260 MAX_IPC_QUEUE_BYTES_PEAK=524288 MAX_IPC_QUEUE_MESSAGES_PEAK=768 ./test_hello.sh` passes (full rebuild); latest run observed `ipc_queue_bytes_peak=3298`, `ipc_queue_messages_peak=40` |
| WP-M6.2 Rendezvous direct-transfer fast path | M6 | IN PROGRESS (recv wait now tracks userspace buffer metadata; endpoint send path attempts direct copy into waiting receiver and stores delivery metadata; `sys_recv` now consumes direct-delivery completion after wake and returns success instead of forced retry path; direct-path telemetry counter added and SLO-gated in harness. Latest increment hardens/optimizes the direct path: small user payloads for `send/call` now stay inline on stack before enqueue/direct copy (no heap allocation in the hot path), and rendezvous waiter scan no longer gets blocked by one incompatible waiter because retryable waiters are rotated and other armed receivers are still considered in the same send.) | `kernel/src/sched/thread.rs`, `kernel/src/sched/thread_manager.rs`, `kernel/src/syscall/handlers.rs`, `kernel/src/ipc/endpoint.rs`, `kernel/src/telemetry.rs`, `test_hello.sh`, `scripts/harness_cases.conf` | `MARKER_MODE=m6_ipc_rendezvous CLUU_BOOTBOOT_ENV=cluu.ipc_direct=1 TEST_COMMAND_REPEAT=1 RUN_WAIT=24 MIN_EXIT_COOKIES=1 MAX_IPC_WAIT_P95_MS=16 MAX_IPC_WAIT_P99_MS=16 MAX_IPC_SCAN_AVG_STEPS_X100=260 MAX_IPC_QUEUE_BYTES_PEAK=262144 MAX_IPC_QUEUE_MESSAGES_PEAK=384 ./test_hello.sh` passes (full rebuild); latest run observed `shell_ready_s=9`, `ipc_direct_deliveries=96`, `ipc_wait_p95_ms=4`, `ipc_scan_avg_steps_x100=156`, `ipc_queue_bytes_peak=2481` |
| WP-M6.3 Shared-ring bulk transfer library over `SpaceGrant` | M6 | IN PROGRESS (P0.3 userspace integration landed: VFS protocol now includes `VFS_RING_SETUP` + `VFS_READ_RING`, server manages per-client ring sessions in a dedicated mapped pool, `libcluu::fs::client` exposes ring setup/read helpers, and shell `ringio` probe exercises the path end-to-end; ring setup is hardened against repeated remap/regrant attempts; still pending: integrate the same ring path into general POSIX read/write hot paths and procmgr data-plane fetches) | `userspace/libcluu/src/ipc.rs`, `userspace/libcluu/src/fs/protocol.rs`, `userspace/libcluu/src/fs/client.rs`, `userspace/vfs/src/main.rs`, `userspace/shell/src/commands.rs`, `test_hello.sh`, `scripts/harness_cases.conf` | `MARKER_MODE=m6_ring_io CLUU_SHELL_AUTOSTART_CMD=ringio CLUU_BOOTBOOT_ENV=cluu.ipc_direct=1 RUN_WAIT=20 ./test_hello.sh` passes with full rebuild; latest run shows `shell_ready_s=9` and `ringio: PASS path=/bin/hello bytes=262144 rounds=16 notify_seq=16` |
| WP-M6.4 IPC SLO rebaseline + guardrails | M6 | DONE (kernel telemetry now tracks IPC queue bytes/messages current+peak in addition to direct deliveries; harness enforces per-mode thresholds for `m6_ipc_compact` and `m6_ipc_rendezvous` including wait/scan and queue-peak limits) | `kernel/src/telemetry.rs`, `kernel/src/ipc/endpoint.rs`, `test_hello.sh`, `scripts/harness_cases.conf`, `scripts/harness_matrix.sh` | `scripts/harness_suite.sh --case m6_ipc_compact`, `scripts/harness_suite.sh --case m6_ipc_rendezvous`, and `scripts/harness_suite.sh --case m6_ring_io` all pass with full rebuild-backed images and hard SLO gates |
...
4. Expand L2B from single-job churn to deterministic multi-job transition matrix (`stop/bg/fg` interleavings + async reap races).
5. Execute M6 IPC throughput hardening in order: compact queue storage -> rendezvous fast path -> shared ring for bulk data -> SLO rebaseline.
6. Execute A1 stubs completion (`signal`/`sigaction`/`raise`, `fcntl`) and add dedicated harness probe.
7. Execute B3 spawn hot-path optimization using `benchprobe` as gate.

### M6 implementation constraints (design guardrails)

1. Keep syscall surface unchanged (`Send/Recv/Call/Reply/Yield/Invoke/DebugPrint` only); no new syscall numbers.
2. Keep process policy in userspace; kernel remains thread/object-centric.
3. Preserve sender-auth semantics (`sender_out_ptr` and authenticated caller binding) on every IPC path.
4. Preserve fairness and backpressure invariants (`MAX_QUEUE_LEN`, `MAX_CALL_QUEUE_LEN`, waiter ordering) while adding fast paths.
5. Land each M6 work package as a separate commit and require harness green before moving to next package.
