...
| WP-L2.1 Mutable FS operations + DAC checks | L2A | IN PROGRESS (`write` + append/grow ext2 writes landed; mutable ops wired end-to-end for ext2 backend: `mkdir/rename/rmdir/unlink`; `O_CREAT` path now creates files through VFS->blk->ext2; harness modes `l2_ext2write` + `l2_ext2append` + `l2_ext2mutate` + `l2_ext2unlink` pass; remaining: stronger ownership-style DAC beyond baseline protected-path deny policy) | `userspace/ext2`, `userspace/virtio-blk`, `userspace/vfs`, `userspace/libcluu`, `userspace/shell`, `test_hello.sh` | `MARKER_MODE=l2_ext2write ./test_hello.sh --no-build`, `MARKER_MODE=l2_ext2append ./test_hello.sh --no-build`, `MARKER_MODE=l2_ext2mutate ./test_hello.sh --no-build`, `MARKER_MODE=l2_ext2unlink ./test_hello.sh --no-build` pass |
