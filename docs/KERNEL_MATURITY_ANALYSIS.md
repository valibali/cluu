...
| WP-B3.1 Spawn hot-path performance pass (procmgr/VFS orchestration) | B3 | IN PROGRESS (spawn-path control-plane overhead reduced: cached `procmgr:spawn` lookup in `libcluu` + corrected `PROC_KILL` label; harness now has dedicated `b_spawn_perf` mode. Latest full-build runs show per-process `procmgr:spawn` subscribe/grant handshakes reduced to one cached lookup and `benchprobe` `spawn_wait` moving from ~192.8M baseline cycles to ~159.0M-181.6M cycles depending on run variance; thread-control stage still needs stabilization for strict PASS gating) | `userspace/libcluu/src/posix/process.rs`, `userspace/libcluu/src/registry.rs`, `userspace/c_hello/benchprobe.c`, `test_hello.sh`, `scripts/harness_cases.conf` | `MARKER_MODE=b_spawn_perf ./test_hello.sh` (full build) for spawn metric + cache-hit behavior; keep `perf_benchprobe` for full mixed probe |
