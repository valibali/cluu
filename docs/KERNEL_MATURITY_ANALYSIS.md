...
| WP-L2.2 Minimal signals + shell job control | L2B | IN PROGRESS (canonical-mode `Ctrl-C` interrupt marker + foreground `spawn` `SIGINT` path landed; minimal background job baseline landed via `spawnbg`/`jobs` + async reap notifications (`shell: bg done pid=...`) from nonblocking notify polling; harness modes `l2_sigint` + `l2_jobs` validate both paths; remaining: true foreground/background promotion (`fg`/`bg`) and explicit `SIGCHLD` queue semantics in libc API) | `userspace/tty`, `userspace/shell`, `userspace/procmgr`, `userspace/libcluu`, `userspace/c_hello`, `xtask`, `test_hello.sh`, `scripts/harness_matrix.sh` | `MARKER_MODE=l2_sigint ./test_hello.sh` and `MARKER_MODE=l2_jobs ./test_hello.sh` pass with full rebuild |
