...
| WP-M6.1 IPC compact-message storage (inline fast path) | M6 | PLANNED (replace fixed `[u8; 4096]` queue payload with `QueuedMessage::{Compact, Large}`; compact form stores label/word_count/words + sender metadata inline, large payload remains boxed) | `kernel/src/ipc/endpoint.rs`, `kernel/src/ipc/mod.rs`, `kernel/src/telemetry.rs`, `userspace/libcluu/src/ipc.rs` | `MARKER_MODE=m6_ipc_compact TEST_COMMAND_REPEAT=3 RUN_WAIT=16 ./test_hello.sh` plus memory delta gates (`MAX_DELTA_ENDPOINTS`, queue-byte telemetry) |
