...
| WP-L2.2 Minimal signals + shell job control | L2B | IN PROGRESS (canonical-mode `Ctrl-C` interrupt marker + foreground `spawn` `SIGINT` path landed; background baseline landed via `spawnbg`/`jobs` + async reap notifications (`shell: bg done pid=...`) from nonblocking notify polling; minimal `fg`/`bg` shell controls landed with foreground promotion (`fg`) and explicit running-state acknowledgement (`bg`) for current no-stop model; libc `waitpid` now has explicit queued child-exit semantics with `WNOHANG` support for `SIGCHLD`-style polling and a dedicated userspace probe (`waitprobe`) in harness mode; remaining: stop/continue signal integration and full job-state transitions) | `userspace/tty`, `userspace/shell`, `userspace/procmgr`, `userspace/libcluu`, `userspace/c_hello`, `xtask`, `test_hello.sh`, `scripts/harness_matrix.sh` | `MARKER_MODE=l2_sigint ./test_hello.sh`, `MARKER_MODE=l2_jobs ./test_hello.sh`, `MARKER_MODE=l2_fg ./test_hello.sh`, and `MARKER_MODE=l2_waitpid ./test_hello.sh` pass with full rebuild |
