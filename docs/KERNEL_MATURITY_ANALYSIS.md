...
| WP-L2.2 Minimal signals + shell job control | L2B | IN PROGRESS (canonical-mode `Ctrl-C` interrupt marker + foreground `spawn` `SIGINT` path landed; background baseline landed via `spawnbg`/`jobs` + async reap notifications (`shell: bg done pid=...`) from nonblocking notify polling; minimal `fg`/`bg` shell controls landed with foreground promotion (`fg`) and explicit running-state acknowledgement (`bg`) for current no-stop model; harness modes `l2_sigint` + `l2_jobs` + `l2_fg` validate all landed paths; remaining: explicit libc-visible `SIGCHLD` queue semantics and future stop/continue integration) | `userspace/tty`, `userspace/shell`, `userspace/procmgr`, `userspace/libcluu`, `userspace/c_hello`, `xtask`, `test_hello.sh`, `scripts/harness_matrix.sh` | `MARKER_MODE=l2_sigint ./test_hello.sh`, `MARKER_MODE=l2_jobs ./test_hello.sh`, and `MARKER_MODE=l2_fg ./test_hello.sh` pass with full rebuild |
