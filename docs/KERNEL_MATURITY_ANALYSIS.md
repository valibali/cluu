...
| WP-B3.1 Spawn hot-path performance pass (procmgr/VFS orchestration) | B3 | PLANNED (targeted latency reduction for spawn-heavy shell workflows while preserving userspace process model) | `userspace/procmgr/src/main.rs`, `userspace/vfs/src/main.rs`, `userspace/libcluu/src/posix/process.rs`, `userspace/c_hello/benchprobe.c`, harness SLO tooling | `MARKER_MODE=perf_benchprobe ./test_hello.sh` with 2x spawn latency improvement target from current baseline |
