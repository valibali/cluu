...
| WP-L2.1 Mutable FS operations + DAC checks | L2A | IN PROGRESS (`write` + append/grow ext2 writes landed; mutable directory ops wired end-to-end for ext2 backend: `mkdir/rename/rmdir` + harness mode `l2_ext2mutate`; remaining: robust `unlink` coverage + DAC/ownership policy checks) | `userspace/ext2`, `userspace/virtio-blk`, `userspace/vfs`, `userspace/libcluu`, `userspace/shell`, `test_hello.sh` | `MARKER_MODE=l2_ext2write ./test_hello.sh --no-build`, `MARKER_MODE=l2_ext2append ./test_hello.sh --no-build`, and `MARKER_MODE=l2_ext2mutate ./test_hello.sh --no-build` pass |
