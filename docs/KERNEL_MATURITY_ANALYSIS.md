...
| WP-L2.1 Mutable FS operations + DAC checks | L2A | IN PROGRESS (`write` path landed: in-place + append/grow ext2 writes with block allocation, VFS/blk plumbing, harness modes `l2_ext2write` + `l2_ext2append`; remaining: create/unlink/rename/rmdir + DAC checks) | `userspace/ext2`, `userspace/virtio-blk`, `userspace/vfs`, `userspace/libcluu`, `userspace/shell`, `test_hello.sh` | `MARKER_MODE=l2_ext2write ./test_hello.sh --no-build` and `MARKER_MODE=l2_ext2append ./test_hello.sh --no-build` pass |
