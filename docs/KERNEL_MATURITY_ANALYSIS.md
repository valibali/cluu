...
| WP-L2.3 Real `mmap` allocator + `mprotect` | L2C | IN PROGRESS (userspace mmap now uses tracked first-fit allocation with overlap checks and free-hole reuse; `munmap` validates exact tracked regions; `mprotect` now routes through kernel-backed `InvokeOp::SpaceProtect` page retagging before updating userspace metadata; dedicated userspace probe + harness marker mode validates map/unmap/reuse/protection behavior) | `kernel/src/token/mod.rs`, `kernel/src/syscall/handlers.rs`, `userspace/libcluu/src/syscall.rs`, `userspace/libcluu/src/posix/memory.rs`, `userspace/c_hello/mmapprobe.c`, `xtask/src/main.rs`, `test_hello.sh`, `scripts/harness_matrix.sh`, kernel VM path | `MARKER_MODE=l2_mmap ./test_hello.sh` passes with full rebuild (`mmapprobe: PASS ...` markers) |
