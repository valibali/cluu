...
| WP-M6.2 Rendezvous direct-transfer fast path | M6 | PLANNED (if receiver already armed on endpoint, bypass queue and transfer directly sender->receiver userspace buffers; keep queue fallback for async) | `kernel/src/syscall/handlers.rs`, `kernel/src/ipc/endpoint.rs`, `kernel/src/sched/*`, `kernel/src/telemetry.rs` | `MARKER_MODE=m6_ipc_rendezvous TEST_COMMAND_REPEAT=3 RUN_WAIT=16 ./test_hello.sh` with reduced `ipc_wait_p95_ms`/`ipc_scan_avg_steps_x100` vs M5 baseline |
