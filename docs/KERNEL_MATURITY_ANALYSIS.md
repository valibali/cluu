...
| WP-M6.3 Shared-ring bulk transfer library over `SpaceGrant` | M6 | PLANNED (introduce libcluu shared ring abstraction for VFS/console bulk I/O; IPC carries descriptors/doorbells only) | `userspace/libcluu/src/ipc.rs`, `userspace/libcluu/src/posix/*`, `userspace/vfs/src/main.rs`, `userspace/console/src/*`, `kernel/src/syscall/handlers.rs` (`SpaceGrant` checks) | `MARKER_MODE=m6_ring_io ./test_hello.sh` + throughput markers from VFS read/write loops |
