...
| WP-L2.2 Minimal signals + shell job control | L2B | IN PROGRESS (canonical-mode `Ctrl-C` now emits interrupt marker from TTY; shell foreground `spawn` now multiplexes child-exit + stdin and sends `SIGINT`/`PROC_KILL` to procmgr on interrupt; harness mode `l2_sigint` validates interrupt path; remaining: `SIGCHLD`-style async notifications and basic background/foreground job table builtins) | `userspace/tty`, `userspace/shell`, `userspace/procmgr`, `userspace/libcluu`, `userspace/c_hello`, `xtask`, `test_hello.sh` | `MARKER_MODE=l2_sigint ./test_hello.sh` passes and logs `spawn: SIGINT pid=` + `procmgr: killed pid` |
