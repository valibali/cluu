...
| WP-L2.2 Minimal signals + shell job control | L2B | IN PROGRESS (canonical-mode `Ctrl-C` interrupt marker + foreground `spawn` `SIGINT` path landed; background baseline landed via `spawnbg`/`jobs` + async reap notifications (`shell: bg done pid=...`) from nonblocking notify polling; kernel-backed thread suspend/resume invoke path is now wired and procmgr routes `SIGSTOP`/`SIGCONT` non-destructively while keeping terminate signals destructive; shell job model now tracks `running/stopped` state with `stop`, `bg`, and `fg` continue-on-foreground behavior; libc `waitpid` now has explicit queued child-exit semantics with `WNOHANG` support for `SIGCHLD`-style polling and a dedicated userspace probe (`waitprobe`) in harness mode; remaining: broaden stop/resume harness coverage and stabilize full job-state transition matrix) | `userspace/tty`, `userspace/shell`, `userspace/procmgr`, `userspace/libcluu`, `kernel/src/sched/*`, `kernel/src/syscall/handlers.rs`, `userspace/c_hello`, `xtask`, `test_hello.sh`, `scripts/harness_matrix.sh` | `MARKER_MODE=l2_sigint ./test_hello.sh`, `MARKER_MODE=l2_jobs ./test_hello.sh`, `MARKER_MODE=l2_fg ./test_hello.sh`, `MARKER_MODE=l2_stop ./test_hello.sh`, and `MARKER_MODE=l2_waitpid ./test_hello.sh` pass with full rebuild |
