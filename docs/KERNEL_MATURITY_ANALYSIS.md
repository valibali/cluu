...
| WP-B3.1 Spawn hot-path performance pass (procmgr/VFS orchestration) | B3 | IN PROGRESS (spawn-path control-plane overhead reduced: cached `procmgr:spawn` lookup in `libcluu` + corrected `PROC_KILL` label; harness now has dedicated `b_spawn_perf` mode. Added timestamped spawn stage traces in procmgr (`spawn_request`, `space_create_done`, `elf_fetch_start/done`, `map_segments_done`, `stack_map_done`, `thread_start_done`, `reply_sent`) and VFS (`map_elf_trace` request/cache/mapped/reply). Full-build measurement now quantifies dominant hot spots: `/bin/noop` spawn trace completes in ~65.6M cycles on a warm path while cold-cache ELF fetch paths (e.g. first `/bin/sleepy`) can exceed ~5.3B cycles; `benchprobe spawn_wait` still shows run-to-run variance (~186.9M-240.7M in recent runs), so next B3 step is cache warmup/metadata reuse stabilization) | `userspace/libcluu/src/posix/process.rs`, `userspace/libcluu/src/registry.rs`, `userspace/procmgr/src/main.rs`, `userspace/vfs/src/main.rs`, `userspace/c_hello/benchprobe.c`, `test_hello.sh`, `scripts/harness_cases.conf` | `MARKER_MODE=b_spawn_perf ./test_hello.sh` (full build) for spawn metric + stage-trace capture; keep `perf_benchprobe` for full mixed probe |
