...
| WP-M6.1 IPC compact-message storage (inline fast path) | M6 | IN PROGRESS (kernel queue storage no longer uses fixed 4KB per message: `EndpointMessage` now stores either inline small payload (<=64B) or exact-size boxed payload; send/call user-copy paths now allocate by message length instead of unconditional 4KB stack buffers; added `m6_ipc_compact` harness mode baseline) | `kernel/src/ipc/endpoint.rs`, `test_hello.sh`, `scripts/harness_cases.conf` | `MARKER_MODE=m6_ipc_compact TEST_COMMAND_REPEAT=1 MIN_EXIT_COOKIES=1 RUN_WAIT=12 ./test_hello.sh --no-build` passes after full rebuild; next gate: add queue-byte telemetry + tighten memory delta thresholds |
| WP-M6.2 Rendezvous direct-transfer fast path | M6 | IN PROGRESS (recv wait now tracks userspace buffer metadata; endpoint send path attempts direct copy into waiting receiver and stores delivery metadata; `sys_recv` now consumes direct-delivery completion after wake and returns success instead of forced retry path; direct-path telemetry counter added) | `kernel/src/sched/thread.rs`, `kernel/src/sched/thread_manager.rs`, `kernel/src/syscall/handlers.rs`, `kernel/src/ipc/endpoint.rs`, `kernel/src/telemetry.rs`, `test_hello.sh`, `scripts/harness_cases.conf` | `MARKER_MODE=m6_ipc_rendezvous TEST_COMMAND_REPEAT=1 MIN_EXIT_COOKIES=6 RUN_WAIT=16 ./test_hello.sh --no-build` should pass with `ipc_direct_deliveries>=1` |
| WP-M6.3 Shared-ring bulk transfer library over `SpaceGrant` | M6 | IN PROGRESS (P0.3 userspace integration landed: VFS protocol now includes `VFS_RING_SETUP` + `VFS_READ_RING`, server manages per-client ring sessions in a dedicated mapped pool, `libcluu::fs::client` exposes ring setup/read helpers, and shell `ringio` probe exercises the path end-to-end; still pending: integrate the same ring path into general POSIX read/write hot paths and procmgr data-plane fetches) | `userspace/libcluu/src/ipc.rs`, `userspace/libcluu/src/fs/protocol.rs`, `userspace/libcluu/src/fs/client.rs`, `userspace/vfs/src/main.rs`, `userspace/shell/src/commands.rs`, `test_hello.sh`, `scripts/harness_cases.conf` | `env -u CLUU_SHELL_AUTOSTART_CMD MARKER_MODE=m6_ring_io RUN_WAIT=12 ./test_hello.sh` passes with full rebuild; latest run shows `shell_ready_s=9` and `ringio: PASS path=/bin/hello bytes=262144 rounds=16 notify_seq=16` |
| WP-M6.4 IPC SLO rebaseline + guardrails | M6 | PLANNED (re-baseline latency/scan/memory SLOs after fast paths, fail CI on regressions, retain fairness invariants) | `kernel/src/telemetry.rs`, `test_hello.sh`, `scripts/harness_matrix.sh` | `cargo xtask harness-matrix` includes `m6_*` modes and hard limits for wait/scan/queue-bytes |
