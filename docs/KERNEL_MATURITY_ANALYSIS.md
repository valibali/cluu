...
| WP-L2.3 Real `mmap` allocator + `mprotect` | L2C | IN PROGRESS (userspace mmap now uses tracked first-fit allocation with overlap checks and free-hole reuse; `munmap`/`mprotect` now validate exact tracked regions and keep protection metadata, with explicit note that kernel-side PTE retagging is still missing; dedicated userspace probe + harness marker mode added for map/unmap/reuse/protection validation) | `userspace/libcluu/src/posix/memory.rs`, `userspace/c_hello/mmapprobe.c`, `xtask/src/main.rs`, `test_hello.sh`, `scripts/harness_matrix.sh`, kernel VM path | `MARKER_MODE=l2_mmap ./test_hello.sh` should pass with full rebuild (`mmapprobe: PASS ...` markers) |
