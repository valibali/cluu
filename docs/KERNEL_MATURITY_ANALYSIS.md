...
| WP-M1.1 Fail-closed manifest verify gate | M1 | DONE (userspace+build gate), kernel verifier pending | `userspace/init` (mandatory manifest + hash/rights enforcement), `xtask` (manifest generation), `kernel/src/bootstrap.rs` (pending verifier module) | boot fails if manifest missing/invalid/mismatched in init; next step is mandatory kernel-side verification/signature root-of-trust |
