...
| WP-L2.3 Real `mmap` allocator + `mprotect` | L2C | IN PROGRESS (userspace mmap now uses tracked first-fit allocation with overlap checks and free-hole reuse; `munmap`/`mprotect` now validate exact tracked regions and keep protection metadata, with explicit note that kernel-side PTE retagging is still missing) | `userspace/libcluu/src/posix/memory.rs`, kernel VM path | dedicated mmap map/unmap/reuse/protection harness mode still pending; full rebuild harness (`MARKER_MODE=l2_jobmix`) passes for regression safety |
