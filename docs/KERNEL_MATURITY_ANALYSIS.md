...
| WP-M1.1 Fail-closed manifest verify gate | M1 | IN PROGRESS (userspace gate) | `userspace/init` (hash+rights enforcement when manifest exists), `kernel/src/bootstrap.rs` (pending verifier module) | boot fails on manifest hash/rights mismatch when manifest present; next step is mandatory kernel-side verification |
